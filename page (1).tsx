    ...
    reader.onload = () => {
      try {
        ...
        if (idx.handle === -1 || idx.name === -1 || idx.price === -1) {
          setErrorMsg("Le CSV doit avoir au moins les colonnes : handle, nom, prix");
          setPreview([]);
          return;
        }
        ...
        const products: Product[] = rows.slice(1).map((r) => ({
          handle: r[idx.handle] || "",
          name: r[idx.name] || "",
          price: parseInt((r[idx.price] || "0").replace(/[^\d]/g, ""), 10) || 0,
          oldPrice: idx.oldPrice > -1 ? parseInt((r[idx.oldPrice] || "0").replace(/[^\d]/g, ""), 10) || 0 : 0,
